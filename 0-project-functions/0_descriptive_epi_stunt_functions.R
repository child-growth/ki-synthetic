...
summary.prev.haz <- function(d, severe.stunted=F, method="REML"){
  ...
  prev.cohort=as.data.frame(rbindlist(prev.cohort, fill=T))
  ...
  prev.res=as.data.frame(rbindlist(prev.res, fill=T))
...
summary.ci <- function(d,  severe.stunted=F, birthstrat=F,
  ...
  ci.res=as.data.frame(rbindlist(ci.res, fill=T))
  ci.res = ci.res %>%
    mutate(est=est*100, lb=lb*100, ub=ub*100)
...
summary.haz <- function(d, method="REML", nmeas_threshold = 50){
  ...
  haz.cohort=as.data.frame(rbindlist(haz.cohort), fill=T)
  ...
  haz.res=as.data.frame(rbindlist(haz.res, fill=T))
...
summary.haz.age.sex <- function(d, method="REML"){
  ...
  haz.cohort.female.df = as.data.frame(rbindlist(haz.cohort.female, fill=T)) %>%
    mutate(sex = "Female")
  
  haz.cohort.male.df = as.data.frame(rbindlist(haz.cohort.male, fill=T)) %>%
    mutate(sex = "Male")
  ...
  haz.df.female = as.data.frame(rbindlist(haz.res.female, fill=T)) %>%
    mutate(sex = "Female")
  
  haz.df.male = as.data.frame(rbindlist(haz.res.male, fill=T)) %>%
    mutate(sex = "Male")
