...
The synthetic data population-attributable risks are generally lower than the real estimates.  The fully-synthetic data most closely matches the real-data estimates, but some of the confidence intervals are much higher. We would have made similar inferences for almost all risk factors using the fully synthetic data, but would have concluded that some important risk factors (like maternal weight and education and household wealth are not strongly associated with poor child growth).
...
When comparing all cohort-specific average-treatment effects (rather than just pooled population-attribuable risks of Z-scores at 24 months), the QI datasets match the real-data estimates most closely. 
...
Similar to the adjusted, SuperLearner estimates, the fully-synthetic data most closely matches the real-data estimates, but the synthetic data population-attributable risks are generally lower. 

### Comparison of difference in cohort-specific targeted learning estimates

The median difference between the fully synthetic and real data is near-zero, and overall, the bias is smaller from the fully synthetic data compared to the QI/BC data.
